...
class DopplerDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor] | Tensor:
        image = decode_image(self.images[idx], mode=ImageReadMode.GRAY)
        image = (image > 255 * self.threshold).float()

        mask = None
        if self.masks is not None:
            mask = decode_image(self.masks[idx], mode=ImageReadMode.GRAY)
